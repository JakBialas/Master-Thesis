...
for (subtype in subtypes) {
  for (arm in arms) {
    for (status in c(0, 1)) {
      ...
      subset_data <- df[df$Subtype == subtype & df$arm == arm & df$efs.ind == status, ]
      
      if (nrow(subset_data) > 0) {
        n_sample <- round(0.5 * nrow(subset_data))
        sampled_data <- subset_data[sample(nrow(subset_data), size = n_sample), ]
        df_sampled <- rbind(df_sampled, sampled_data)
      }
    }
  }
}
...
result_list <- mclapply(1:nrow(combs), function(i) {
  ...
  for (j in 1:10) {
    ...
    ControlData <- generate_smote_data(df, 'Control', subtype, percent)
    ...
    RegimenData <- generate_smote_data(df, regimen, subtype, percent)
    if (nrow(RegimenData) == 0) {
      warning(paste("No data for regimen", regimen, "at", percent, "% augmentation"))
      next
    }
    ...
    if (!is.null(res) && length(res) == 2) {
      ...
      warning(paste("Error calculating TES for regimen", regimen, "at", percent, "% augmentation"))
    }
  }
...
if (!dir.exists("plots_SMOTE")) dir.create("plots_SMOTE")
  ...
  for (regimen in unique(results$Regimen)) {
    ...
    if (nrow(subset_results) > 0) {
      ...
      plot_TES <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(SMOTE) TES Dependency, (", subtype, " ", regimen, ")"),
      ...
      plot_variance <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(SMOTE) TES Variance, (", subtype, " ", regimen, ")"),
      ...
      plot_TES_filename <- paste0("plots_SMOTE/TES_", subtype, "_", regimen, ".png")
      plot_variance_filename <- paste0("plots_SMOTE/Variance_", subtype, "_", regimen, ".png")
...
for (percent in augmentation_percents) {
  cat("Processing augmentation at:", percent, "%\n")
  flush.console()
  
  results_selected <- results[results$Percent == percent, ]
  results_rmst_selected <- calculate_rmst_with_TES(df, results_selected)
  
  if (nrow(results_rmst_selected) == 0) {
    warning(paste("No RMST data for augmentation", percent, "%"))
    next
  }
  
  pearson_corr <- cor(results_rmst_selected$TES_avg, results_rmst_selected$Delta_RMST, method = "pearson")
  spearman_corr <- cor(results_rmst_selected$TES_avg, results_rmst_selected$Delta_RMST, method = "spearman")
  
  lm_model <- lm(Delta_RMST ~ TES_avg, data = results_rmst_selected)
  slope <- coef(lm_model)[2]
  intercept <- coef(lm_model)[1]
  
  plot_selected_percent <- ggplot(results_rmst_selected, aes(x = TES_avg, y = Delta_RMST, color = Subtype, label = Regimen)) +
    geom_point(size = 3) +
    geom_text(vjust = -0.5, hjust = 0.5, size = 3) +
    geom_smooth(method = "lm", se = TRUE, color = "black", linetype = "dashed") +
    labs(title = paste("Delta RMST vs TES for", percent, "% Augmentation (SMOTE)"),
         x = "TES", y = "Delta RMST") +
    theme_light() +
  ...
  plot_filename <- paste0("plots_SMOTE/Delta_RMST_vs_TES_", percent, "_percent.png")
  ggsave(plot_filename, plot = plot_selected_percent, width = 8, height = 6, dpi = 300)
}
...
for (percent in augment_levels) {
  selected_results <- results[results$Percent == percent, ]
  rmst_data <- calculate_rmst_with_TES(df, selected_results)
...
plot_metric <- function(data, y_col, y_label) {
  p <- ggplot(data, aes(x = Percent, y = .data[[y_col]])) +
    ...
    labs(title = paste(y_label, "vs Augmentation (SMOTE)"),
...
ggsave("plots_SMOTE/Correlation_and_Regression_vs_Augmentation_2x2.png", plot = grid_plot, width = 12, height = 8, dpi = 300)
######

cat("The plots were saved in the 'plots_SMOTE' folder.\n")
flush.console()
