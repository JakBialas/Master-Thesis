...
result_list <- mclapply(1:nrow(combs), function(i) {
  ...
  cat("Processing (Hybrid): Subtype =", subtype, ", Percent =", percent, "%, Regimen =", regimen, "\n")
  flush.console()
  ...
  for (j in 1:10) {
    cat("Running repetition", j, "(Hybrid), augmentation at:", percent, "%\n")
    flush.console()

    ControlData_stage1 <- generate_smoteCox_data(df, 'Control', subtype, 100)%>%
     rename(rcbindex = feature1, efs.time = time, efs.ind = event)
    if (nrow(ControlData_stage1) == 0) {
      warning("No data after stage1 for Control, I'm using the original")
      ControlData_stage1 <- df[df$Group == 'Control' & df$Subtype == subtype, ]
    }
    ControlData_stage1$arm <- 'Control'
    ControlData_stage1$Subtype <- subtype
    ControlData <- generate_smoteCox_data(ControlData_stage1, 'Control', subtype, percent)
    ...
    RegimenData_stage1 <- generate_smoteCox_data(df, regimen, subtype, 100) %>%
     rename(rcbindex = feature1, efs.time = time, efs.ind = event)
    if (nrow(RegimenData_stage1) == 0) {
      warning(paste("No data after stage1 for regimen", regimen))
      RegimenData_stage1 <- df[df$Group == regimen & df$Subtype == subtype, ]
    }
    RegimenData_stage1$arm <- regimen
    RegimenData_stage1$Subtype <- subtype
    RegimenData <- generate_smoteCox_data(RegimenData_stage1, regimen, subtype, percent)
    if (nrow(RegimenData) == 0) {
      warning(paste("No data for the regimen", regimen, "at", percent, "% augmentation"))
      next
    }
...
if (!dir.exists("plots_Hybridv2")) dir.create("plots_Hybridv2")
  ...
  for (regimen in unique(results$Regimen)) {
    ...
    if (nrow(subset_results) > 0) {
      ...
      plot_TES <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(Hybrid) TES Dependency, (", subtype, " ", regimen, ")"),
             subtitle = paste("Slope =", sprintf("%.10f", slope_TES), ", Intercept =", sprintf("%.10f", intercept_TES)),
      ...
      plot_variance <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(Hybrid) TES Variance, (", subtype, " ", regimen, ")"),
             subtitle = paste("Slope =", sprintf("%.10f", slope_variance), ", Intercept =", sprintf("%.10f", intercept_variance)),
             x = "Augmentation percentage", y = "TES Variance") +
        theme_light()

      plot_TES_filename <- paste0("plots_Hybridv2/TES_", subtype, "_", regimen, ".png")
      plot_variance_filename <- paste0("plots_Hybridv2/Variance_", subtype, "_", regimen, ".png")
...
for (augmentation_percent in augmentation_percents) {
  ...
  plot <- ggplot(results_subset, aes(x = TES_avg, y = Delta_RMST, color = Subtype, label = Regimen)) +
    ...
    labs(title = paste("Delta RMST vs TES for", augmentation_percent, "% Augmentation (Hybrid)"),
         x = "TES", y = "Delta RMST") +
    theme_light() +
  ...
  filename <- paste0("plots_Hybridv2/Delta_RMST_vs_TES_", augmentation_percent, "_percent.png")
...
plot_metric <- function(data, y_col, y_label) {
  p <- ggplot(data, aes(x = Percent, y = .data[[y_col]])) +
    ...
    labs(title = paste(y_label, "vs Augmentation (Hybrid)"),
...
ggsave("plots_Hybridv2/Correlation_and_Regression_vs_Augmentation_2x2.png", plot = grid_plot, width = 12, height = 8, dpi = 300)
######


cat("The plots were saved in the 'plots_Hybridv2' folder.\n")
flush.console()
