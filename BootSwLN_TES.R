...
result_list <- mclapply(1:nrow(combs), function(i) {
  ...
  for (j in 1:10) {
    ...
    ControlData <- augment_data_BootSwLN(df, 'Control', subtype, percent, 1)
    ...
    RegimenData <- augment_data_BootSwLN(df, regimen, subtype, percent, 1)
...
if (!dir.exists("plots_BootSwLN")) dir.create("plots_BootSwLN")
  ...
  for (regimen in unique(results$Regimen)) {
    ...
    if (nrow(subset_results) > 0) {
      ...
      plot_TES <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(BootStrap with LogNorm) TES Dependency, (", subtype, " ", regimen, ")"),
      ...
      plot_variance <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(BootStrap with LogNorm) TES Variance, (", subtype, " ", regimen, ")"),
      ...
      plot_TES_filename <- paste0("plots_BootSwLN/TES_", subtype, "_", regimen, ".png")
      plot_variance_filename <- paste0("plots_BootSwLN/Variance_", subtype, "_", regimen, ".png")
...
for (percent in augmentation_percents) {
  ...
  plot_selected_percent <- ggplot(results_rmst_selected, aes(x = TES_avg, y = Delta_RMST, color = Subtype, label = Regimen)) +
    ...
    labs(title = paste("Delta RMST vs TES for", percent, "% Augmentation (BootStrap with LogNorm)"),
         x = "TES", y = "Delta RMST") +
    theme_light() +
  ...
  plot_filename <- paste0("plots_BootSwLN/Delta_RMST_vs_TES_", percent, "_percent.png")
...
plot_metric <- function(data, y_col, y_label) {
  p <- ggplot(data, aes(x = Percent, y = .data[[y_col]])) +
    ...
    labs(title = paste(y_label, "vs Augmentation (BootStrap with LogNorm)"),
...
ggsave("plots_BootSwLN/Correlation_and_Regression_vs_Augmentation_2x2.png", plot = grid_plot, width = 12, height = 8, dpi = 300)
######

cat("The plots were saved in the 'plots_BootSwLN' folder.\n")
flush.console()
