...
result_list <- mclapply(1:nrow(combs), function(i) {
  ...
  for (j in 1:10) {
    ...
    ControlData <- generate_smoteCox_data2Ev(df, 'Control', subtype, percent)
    ...
    RegimenData <- generate_smoteCox_data2Ev(df, regimen, subtype, percent)
...
if (!dir.exists("plots_SMOTECox2Ev")) dir.create("plots_SMOTECox2Ev")
  ...
  for (regimen in unique(results$Regimen)) {
    ...
    if (nrow(subset_results) > 0) {
      ...
      plot_TES <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(SMOTECox2Ev) TES Dependency, (", subtype, " ", regimen, ")"),
      ...
      plot_variance <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(SMOTECox2Ev) TES Variance, (", subtype, " ", regimen, ")"),
      ...
      plot_TES_filename <- paste0("plots_SMOTECox2Ev/TES_", subtype, "_", regimen, ".png")
      plot_variance_filename <- paste0("plots_SMOTECox2Ev/Variance_", subtype, "_", regimen, ".png")
...
for (augmentation_percent in augmentation_percents) {
  ...
  plot <- ggplot(results_subset, aes(x = TES_avg, y = Delta_RMST, color = Subtype, label = Regimen)) +
    ...
    labs(title = paste("Delta RMST vs TES for", augmentation_percent, "% Augmentation (SMOTECox2Ev)"),
         x = "TES", y = "Delta RMST") +
    theme_light() +
  ...
  filename <- paste0("plots_SMOTECox2Ev/Delta_RMST_vs_TES_", augmentation_percent, "_percent.png")
...
plot_metric <- function(data, y_col, y_label) {
  p <- ggplot(data, aes(x = Percent, y = .data[[y_col]])) +
    ...
    labs(title = paste(y_label, "vs Augmentation (SMOTECox2Ev)"),
...
ggsave("plots_SMOTECox2Ev/Correlation_and_Regression_vs_Augmentation_2x2.png", plot = grid_plot, width = 12, height = 8, dpi = 300)
######


cat("The plots were saved in the 'plots_SMOTECox2Ev' folder.\n")
flush.console()
