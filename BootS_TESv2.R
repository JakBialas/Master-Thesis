...
if (!dir.exists("plots_Bootstrapv2")) dir.create("plots_Bootstrapv2")
...
for (subtype in unique(results$Subtype)) {
  for (regimen in unique(results$Regimen)) {
    ...
    if (nrow(subset_results) > 0) {
      ...
      plot_TES_filename <- paste0("plots_Bootstrapv2/TES_", subtype, "_", regimen, ".png")
      plot_variance_filename <- paste0("plots_Bootstrapv2/Variance_", subtype, "_", regimen, ".png")
...
for (augmentation_percent in augmentation_percents) {
  ...
  if (nrow(results_subset) == 0) {
    warning(paste("Brak danych dla", augmentation_percent, "% augmentacji"))
    next
  }
  ...
  plot <- ggplot(results_subset, aes(x = TES_avg, y = Delta_RMST, color = Subtype, label = Regimen)) +
    ...
    labs(title = paste("Delta RMST vs TES for", augmentation_percent, "% Augmentation (BootStrap)"),
         x = "TES", y = "Delta RMST") +
    theme_light() +
  ...
  filename <- paste0("plots_Bootstrapv2/Delta_RMST_vs_TES_", augmentation_percent, "_percent.png")
...
ggsave("plots_Bootstrapv2/Correlation_and_Regression_vs_Augmentation_2x2.png", plot = grid_plot, width = 12, height = 8, dpi = 300)
######


cat("The plots were saved in the 'plots_Bootstrapv2' folder.\n")
flush.console()
