...
result_list <- mclapply(1:nrow(combs), function(i) {
  ...
  for (j in 1:10) {
    ...
    ControlData <- augment_data_random(df, 'Control', subtype, percent = percent)
    ...
    RegimenData <- augment_data_random(df, regimen, subtype, percent = percent)
...
if (!dir.exists("plots_RANDOM")) dir.create("plots_RANDOM")
  ...
  for (regimen in unique(results$Regimen)) {
    ...
    if (nrow(subset_results) > 0) {
      ...
      plot_TES <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(RANDOM) TES Dependency, (", subtype, " ", regimen, ")"),
      ...
      plot_variance <- ggplot(subset_results, aes(x = Percent)) +
        ...
        labs(title = paste("(RANDOM) TES Variance, (", subtype, " ", regimen, ")"),
      ...
      plot_TES_filename <- paste0("plots_RANDOM/TES_", subtype, "_", regimen, ".png")
      plot_variance_filename <- paste0("plots_RANDOM/Variance_", subtype, "_", regimen, ".png")
...
for (percent in augmentation_percents) {
  ...
  plot_selected_percent <- ggplot(results_rmst_selected, aes(x = TES_avg, y = Delta_RMST, color = Subtype, label = Regimen)) +
    ...
    labs(title = paste("Delta RMST vs TES for", percent, "% Augmentation (RANDOM)"),
         x = "TES", y = "Delta RMST") +
    theme_light() +
  ...
  plot_filename <- paste0("plots_RANDOM/Delta_RMST_vs_TES_", percent, "_percent.png")
...
plot_metric <- function(data, y_col, y_label) {
  p <- ggplot(data, aes(x = Percent, y = .data[[y_col]])) +
    ...
    labs(title = paste(y_label, "vs Augmentation (RANDOM)"),
...
ggsave("plots_RANDOM/Correlation_and_Regression_vs_Augmentation_2x2.png", plot = grid_plot, width = 12, height = 8, dpi = 300)
######


cat("The plots were saved in the 'plots_RANDOM' folder.\n")
flush.console()
...
summary_results <- results %>%
  ...
  mutate(
    ...
    TES_avg = TES_avg,
    TES_var = TES_var,
    p_value_avg = p_value_avg  # Dodanie p-value sredniej
  )
